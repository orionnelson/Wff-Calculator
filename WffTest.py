...
def run():
    ...
    class WffTest(object):
        ...
        def booleangen(primarray):
            """
    Warning: Useless Text Below


    Row Prim = [letter][islast][operation on array]
    access to the the letter can be done by..
    ...
    primarray[iteratedx][0] = Give primitive or operation symbol. 

    if(primarray[iterated][1]==false)  print the final truth table all the values are in.

    the array thislist is an array of arrays which is then flipped.
    done by.
    By thislist[l-v][q-x] for which
    ~
    *l  = primitive for the generator part. / num primitive variables
    *v = decrases the length from *L backwards to 0.
    *q = number of spaces taken up by truth values [2^n variables].
    *x = the row that the current print is on starts at 2**L and decreases everytime
    the row reaches its first valuse V is done by row number not by index.
    decreases when v==1.

    thislist is shown below:

    [['a',2**L Truth values], ['b',2**L Truth values], ['step num', 2**L]]
    ...
    however the top row I removeved because of formatting issues step number 10 and above was
    making the table hard to read as the spaces did not line up.

    primarray example pictured below
    ['ν', False, [1, 2]]

    # Start psudofunction: add rows to table.
    I dont actually know how to write psudocode but this is just how I planned the last bit out.

    Runs Each iteration of itteratedX...

    trulist=[] adds new array.
    trulist.add(thislist.size()+1) easily finds step number.


    def fand(x,y,thislist):
        for  z in range (1,len(thislist)):
             ans = (((bool)thislist[x-1][z]) and ((bool)thislist[y-1][z]))
             trulist.append( ans)
        return

        
    def fuor(x,y,thislist):
        for  z in range (1,len(thislist)):
             ans = (((bool)thislist[x-1][z]) or ((bool)thislist[y-1][z]))
             trulist.append(ans)
        return

    def fimp(x,y,thislist):
        for  z in range (1,len(thislist)):
             ans = ((not (bool)thislist[x-1][z]) or ((bool)thislist[y-1][z]))
             trulist.append( ans)
        return



    if(primarray[itteratedx][1] in ('¬')): checks for single operation
        select = primarray[itteratedx][3]
        for (bool) item in thislist[select-1][1:]:
            trulist.append( not item)
        
    if(primarray[itteratedx][1] in ('Λ','ν','→')):
        if(primarray[itteratedx][1]==('Λ')):
           fand((primarray[itteratedx][2][0]),(primarray[itteratedx][2][1]),thislist,primarray)
        else if(primarray[itteratedx][1]==('ν')):
           fuor((primarray[itteratedx][2][0]),(primarray[itteratedx][2][1]),thislist,primarray)
        else:
            fimp((primarray[itteratedx][2][0]),(primarray[itteratedx][2][1]),thislist,primarray)
